...
export const generateRandomTrooper = (level: number = 1): Trooper => {
    ...
    // FIX: Standardize human stats. Only HP is fixed to 10 for humans in original game (approx).
    // Let's set a standard base HP for humans.
    if (trooperClass !== 'Recruit' || true) { // All humans for now
        baseStats.maxHp = 10;
        baseStats.hp = 10;
